...
app.get('/insertStats', async(req, res) => {
  const data = {buss_id: `${req.params.buss_id}`, stats:{monthly_report: 0, monthly_paid: 0, avg_paid: 0, mmm_reports: 0, mmm_paid: 0, mmm_avg: 0}, report_counts:{open: 0, resolved: 0}, report_cvss:{NA:0, dups: 0, info: 0, medium: 0, high: 0, critical: 0}}
  // const jsondata = JSON.stringify(data);
  const user = new BussStats(data);
  const result = await user.save();
  res.status(200).json({ "buss_id": `${result.buss_id}`,
	"stats" : {
